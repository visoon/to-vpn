...
class PacketTunnelProvider: NEPacketTunnelProvider, DNSResolverDelegate{
    ...
    override func startTunnel(options: [String : NSObject]?, completionHandler: @escaping (Error?) -> Void) {
        ...
        DDLog.add(DDASLLogger.sharedInstance, with: DDLogLevel.error)
        ...
        setTunnelNetworkSettings(networkSettings) {
            ...
            if self.enablePacketProcessing {
                ...
                let ipAddress = NEKit.IPAddress(fromString: "114.114.114.114")!
                let port = NEKit.Port(port: 53)
                let resolver = UDPDNSResolver(address: ipAddress, port: port)
